...
import com.dounine.catwechat.behaviors.engine.AccessTokenBehavior.Token
import com.dounine.catwechat.behaviors.engine.{
  AccessTokenBehavior,
  JSApiTicketBehavior
}
import com.dounine.catwechat.model.models.WechatModel.LoginParamers
...
import com.dounine.catwechat.router.routers.SuportRouter
...
//              )
        ...
        } else if (message.eventKey.contains("subscribe_msg_popup_event")) {
          message.subscribeMsgPopupEvent
            .foreach(popups => {
              popups
                .filter(_.subscribeStatusString == "accept")
                .foreach(i => {
                  Source
                    .single(
                      message.fromUserName
                    ...
                    .flatMapConcat(result => {
                      Source
                        ...
                        .via(
                          BreakDownStream.openidAndAccountLatest()
                        )
                        .map(result -> _)
                    })
                    .flatMapConcat {
