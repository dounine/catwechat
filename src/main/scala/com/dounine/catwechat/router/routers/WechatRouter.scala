...
import com.dounine.catwechat.behaviors.engine.AccessTokenBehavior
...
import com.dounine.catwechat.model.types.service.{LogEventKey, PayStatus}
                    ...
                    extractRequest {
                      request =>
                        ...
                        val params: String = Map(
                          "appid" -> "wxa1e0369c9ff65e3a",
                          "redirect_uri" -> URLEncoder.encode(
                            (scheme + "://" + domain) + s"/?ccode=${ccode}&platform=douyin&appid=wxa1e0369c9ff65e3a",
                            "utf-8"
                          ),
                          "response_type" -> "code",
                          "scope" -> "snsapi_base",
                          "state" -> "wxa1e0369c9ff65e3a"
                        ).map(i => s"${i._1}=${i._2}")
                          .mkString("&")
                        redirect(
